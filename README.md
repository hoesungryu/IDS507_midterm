...
### Q1. `data` 폴더에서 `train_data_1.csv`와 `train_data_2.csv`를 데이터프레임으로 불러온 후 두개의 데이터프레임을 상하(axis=0)로 합쳐서 `df` 변수로 저장  
...
(여기서 encoding 이란 문자열을 숫자로 바꾸는 것 입니다. 즉 low, mid, high 를 숫자로 바꿔주어야합니다. 
해당 코드는 Lab02 Label encoding 부분을 참조하시면 될 것 같습니다.)
...
### Q9. `모델의 예측값`(ACC,ROC-AUC 둘중하나값) 을 프린트하고, `Roc curve`를 그리기
